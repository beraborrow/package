...
[Home](./index.md) &gt; [@beraborrow/lib-base](./lib-base.md) &gt; [TroveAdjustmentParams](./lib-base.troveadjustmentparams.md)

## TroveAdjustmentParams type

Parameters of an [adjustTrove()](./lib-base.transactableberaborrow.adjusttrove.md) transaction.
...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowNECT` and `repayNECT` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowNECT? </td> <td> T </td> <td> <i>(Optional)</i> The amount of NECT that's borrowed. </td> </tr>
