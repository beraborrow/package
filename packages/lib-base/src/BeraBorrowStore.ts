...
import { FrontendStatus } from "./ReadableBeraBorrow";
...
export interface BeraBorrowStoreBaseState {
   ...
   *
   * @remarks
   * See {@link ReadableBeraBorrow.getCollateralSurplusBalance | getCollateralSurplusBalance()} for
   * more information.
   */
   ...
   * {@link BeraBorrowStoreDerivedState.trove | trove}.
   */
...
/**
 * State variables derived from {@link BeraBorrowStoreBaseState}.
 *
 * @public
 */
export interface BeraBorrowStoreDerivedState {
   ...
   *
   * @remarks
   * Note that the actual rate paid by a redemption transaction will depend on the amount of NECT
   * being redeemed.
   *
...
/**
 * Type of {@link BeraBorrowStore}'s {@link BeraBorrowStore.state | state}.
 *
 * @remarks
 * It combines all properties of {@link BeraBorrowStoreBaseState} and {@link BeraBorrowStoreDerivedState}
 * with optional extra state added by the particular `BeraBorrowStore` implementation.
 *
...
/**
 * Parameters passed to {@link BeraBorrowStore} listeners.
 *
 * @remarks
 * Use the {@link BeraBorrowStore.subscribe | subscribe()} function to register a listener.
 ...
 *
 * @remarks
 * The type parameter `T` may be used to type extra state added to {@link BeraBorrowStoreState} by the
 * subclass.
 *
 * Implemented by {@link @beraborrow/lib-ethers#BlockPolledBeraBorrowStore}.
 *
 * @public
 */
export abstract class BeraBorrowStore<T = unknown> {
   ...
   *
   * @remarks
   * See {@link BeraBorrowStore.start | start()}.
   */
  onLoaded?: () => void;
  ...
  private _baseState?: BeraBorrowStoreBaseState;
  private _derivedState?: BeraBorrowStoreDerivedState;
  private _extraState?: T;

  private _updateTimeoutId: ReturnType<typeof setTimeout> | undefined;
  private _listeners = new Set<(params: BeraBorrowStoreListenerParams<T>) => void>();
   ...
   * {@link BeraBorrowStore.onLoaded | onLoaded} to get a callback when this happens.
   *
   * See {@link BeraBorrowStoreState} for the list of properties returned.
   */
  ...
  /**
   * Start monitoring the blockchain for BeraBorrow state changes.
   *
   * @remarks
   * The {@link BeraBorrowStore.onLoaded | onLoaded} callback will be called after the state is fetched
   * for the first time.
   *
   * Use the {@link BeraBorrowStore.subscribe | subscribe()} function to register listeners.
   *
  ...
  private _reduce(
    baseState: BeraBorrowStoreBaseState,
    baseStateUpdate: Partial<BeraBorrowStoreBaseState>
    ...
    return {
      ...
      nectBalance: this._updateIfChanged(
        eq,
        "nectBalance",
        baseState.nectBalance,
        baseStateUpdate.nectBalance
      ),

      pollenBalance: this._updateIfChanged(
        eq,
        "pollenBalance",
        baseState.pollenBalance,
        baseStateUpdate.pollenBalance
      ),
      ...
      remainingLiquidityMiningPOLLENReward: this._silentlyUpdateIfChanged(
        eq,
        baseState.remainingLiquidityMiningPOLLENReward,
        baseStateUpdate.remainingLiquidityMiningPOLLENReward
      ),
      ...
      liquidityMiningPOLLENReward: this._silentlyUpdateIfChanged(
        eq,
        baseState.liquidityMiningPOLLENReward,
        baseStateUpdate.liquidityMiningPOLLENReward
      ),
      ...
      nectInStabilityPool: this._updateIfChanged(
        eq,
        "nectInStabilityPool",
        baseState.nectInStabilityPool,
        baseStateUpdate.nectInStabilityPool
      ),
      ...
      remainingStabilityPoolPOLLENReward: this._silentlyUpdateIfChanged(
        eq,
        baseState.remainingStabilityPoolPOLLENReward,
        baseStateUpdate.remainingStabilityPoolPOLLENReward
      ),
      ...
      pollenStake: this._updateIfChanged(
        equals,
        "pollenStake",
        baseState.pollenStake,
        baseStateUpdate.pollenStake
      ),

      totalStakedPOLLEN: this._updateIfChanged(
        eq,
        "totalStakedPOLLEN",
        baseState.totalStakedPOLLEN,
        baseStateUpdate.totalStakedPOLLEN
      ),
  ...
  }: BeraBorrowStoreBaseState): BeraBorrowStoreDerivedState {
  ...
  private _reduceDerived(
    derivedState: BeraBorrowStoreDerivedState,
    derivedStateUpdate: BeraBorrowStoreDerivedState
  ): BeraBorrowStoreDerivedState {
    return {
  ...
  private _notify(params: BeraBorrowStoreListenerParams<T>) {
