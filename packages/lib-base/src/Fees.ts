 ...
 *
 * @remarks
 * Returned by the {@link ReadableBeraBorrow.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in NECT, multiply the borrowed NECT amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await beraborrow.getFees();
   *
   * const borrowedNECTAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeNECT = borrowingRate.mul(borrowedNECTAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of NECT being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total NECT supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in NECT, multiply the redeemed NECT amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedNECTAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedNECTAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeNECT = redemptionRate.mul(redeemedNECTAmount);
   * ```
   */
