...
import { POLLENStake } from "../src/POLLENStake";

const arbitraryStake = () =>
  fc
    .tuple(fc.float(), fc.float(), fc.float())
    .map(([a, b, c]) => new POLLENStake(Decimal.from(a), Decimal.from(b), Decimal.from(c)));

const nonZeroStake = () => arbitraryStake().filter(({ stakedPOLLEN }) => !stakedPOLLEN.isZero);

describe("POLLENStake", () => {
