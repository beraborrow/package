...
contract('Deploying and funding One Year Lockup Contracts', async accounts => {
  const [beraborrowAG, A, B, C, D, E, F, G, H, I, J] = accounts;
  ...
  beforeEach(async () => {
    ...
    POLLENContracts = await deploymentHelper.deployPOLLENContracts(bountyAddress, lpRewardsAddress, multisig)
  ...
  describe('Deploying LCs', async accounts => {
    it("POLLEN Deployer can deploy LCs through the Factory", async () => {
      ...
      const LCDeploymentTx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: beraborrowAG })
    ...
    it("Anyone can deploy LCs through the Factory", async () => {
      ...
      const LCDeploymentTx_3 = await lockupContractFactory.deployLockupContract(beraborrowAG, oneYearFromSystemDeployment, { from: I })
    ...
    it("POLLEN Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(pollenToken.address, B, oneYearFromSystemDeployment, { from: beraborrowAG })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(pollenToken.address, C, oneYearFromSystemDeployment, { from: beraborrowAG })
    ...
    it("Anyone can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(pollenToken.address, B, oneYearFromSystemDeployment, { from: E })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(pollenToken.address, C, oneYearFromSystemDeployment, { from: F })
      const LC_C_txReceipt = await web3.eth.getTransactionReceipt(LC_C.transactionHash)
    ...
    it("LC deployment stores the beneficiary's address in the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_D = await lockupContractFactory.deployLockupContract(D, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_E = await lockupContractFactory.deployLockupContract(E, oneYearFromSystemDeployment, { from: beraborrowAG })
    ...
    it("LC deployment through the Factory records the LC contract address and deployer as a k-v pair in the Factory", async () => {
      ...
      assert.equal(beraborrowAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_A))
      assert.equal(beraborrowAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_B))
      assert.equal(beraborrowAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_C))
      assert.equal(beraborrowAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_D))
      assert.equal(beraborrowAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_E))
    })
    ...
    it("Direct deployment of LC sets the unlockTime in the LC", async () => {
      ...
      const LC_A = await LockupContract.new(pollenToken.address, A, oneYearFromSystemDeployment, { from: beraborrowAG })
      const LC_B = await LockupContract.new(pollenToken.address, B, '230582305895235', { from: B })
      const LC_C = await LockupContract.new(pollenToken.address, C, dec(20, 18), { from: E })
    ...
    it("LC deployment through the Factory reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      // Deploy 3 LCs through factory
      const LCDeploymentPromise_A = lockupContractFactory.deployLockupContract(A, nearlyOneYear, { from: beraborrowAG })
    ...
    it("Direct deployment of LC reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      const LCDeploymentPromise_A = LockupContract.new(pollenToken.address, A, nearlyOneYear, { from: beraborrowAG })
      const LCDeploymentPromise_B = LockupContract.new(pollenToken.address, B, '37', { from: B })
      const LCDeploymentPromise_C = LockupContract.new(pollenToken.address, C, '43200', { from: E })
  ...
  describe('Funding LCs', async accounts => {
    it("POLLEN transfer from POLLEN deployer to their deployed LC increases the POLLEN balance of the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: beraborrowAG })
      ...
      assert.equal(await pollenToken.balanceOf(LCAddress_D), '0')
      assert.equal(await pollenToken.balanceOf(LCAddress_E), '0')

      // Multisig transfers POLLEN to each LC
      await pollenToken.transfer(LCAddress_A, POLLENEntitlement_A, { from: multisig })
      await pollenToken.transfer(LCAddress_B, POLLENEntitlement_B, { from: multisig })
      await pollenToken.transfer(LCAddress_C, POLLENEntitlement_C, { from: multisig })
      await pollenToken.transfer(LCAddress_D, POLLENEntitlement_D, { from: multisig })
      await pollenToken.transfer(LCAddress_E, POLLENEntitlement_E, { from: multisig })

      assert.equal(await pollenToken.balanceOf(LCAddress_A), POLLENEntitlement_A)
      assert.equal(await pollenToken.balanceOf(LCAddress_B), POLLENEntitlement_B)
      assert.equal(await pollenToken.balanceOf(LCAddress_C), POLLENEntitlement_C)
      assert.equal(await pollenToken.balanceOf(LCAddress_D), POLLENEntitlement_D)
      assert.equal(await pollenToken.balanceOf(LCAddress_E), POLLENEntitlement_E)
    })

    it("POLLEN Multisig can transfer POLLEN to LCs deployed through the factory by anyone", async () => {  
      ...
      // Multisig transfers POLLEN to each LC
      await pollenToken.transfer(LCAddress_A, dec(1, 18), { from: multisig })
      await pollenToken.transfer(LCAddress_B, dec(2, 18), { from: multisig })
      await pollenToken.transfer(LCAddress_C, dec(3, 18), { from: multisig })
      await pollenToken.transfer(LCAddress_D, dec(4, 18), { from: multisig })
      await pollenToken.transfer(LCAddress_E, dec(5, 18), { from: multisig })

      assert.equal(await pollenToken.balanceOf(LCAddress_A), dec(1, 18))
      assert.equal(await pollenToken.balanceOf(LCAddress_B), dec(2, 18))
      assert.equal(await pollenToken.balanceOf(LCAddress_C), dec(3, 18))
      assert.equal(await pollenToken.balanceOf(LCAddress_D), dec(4, 18))
      assert.equal(await pollenToken.balanceOf(LCAddress_E), dec(5, 18))
    })

    // can't transfer POLLEN to any LCs that were deployed directly
  })
  ...
  describe('Withdrawal attempts on funded, inactive LCs immediately after funding', async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers POLLEN to each LC
      await pollenToken.transfer(LC_A.address, POLLENEntitlement_A, { from: multisig })
      await pollenToken.transfer(LC_B.address, POLLENEntitlement_B, { from: multisig })
      await pollenToken.transfer(LC_C.address, POLLENEntitlement_C, { from: multisig })

      assert.equal(await pollenToken.balanceOf(LC_A.address), POLLENEntitlement_A)
      assert.equal(await pollenToken.balanceOf(LC_B.address), POLLENEntitlement_B)
      assert.equal(await pollenToken.balanceOf(LC_C.address), POLLENEntitlement_C)
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawPOLLEN({ from: beneficiary })
    ...
    it("POLLEN multisig can't withraw from a LC which it funded", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: beraborrowAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: beraborrowAG })
      ...
      // POLLEN multisig attempts to withdraw from LCs
      for (LC of LCs) {
        try {
          const withdrawalAttemptTx = await LC.withdrawPOLLEN({ from: multisig })
          assert.isFalse(withdrawalAttemptTx.receipt.status)
        } catch (error) {
          assert.include(error.message, "revert")
        }
    ...
    it("No one can withraw from a LC", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, POLLENEntitlement_A, { from: D })

      // Grab contract objects from deployment tx events
      const LC_A = await th.getLCFromDeploymentTx(deployedLCtx_A)

      // BeraBorrowAG transfers POLLEN to the LC
      await pollenToken.transfer(LC_A.address, POLLENEntitlement_A, { from: multisig })
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawPOLLEN({ from: G })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawPOLLEN({ from: H })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawPOLLEN({ from: I })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
