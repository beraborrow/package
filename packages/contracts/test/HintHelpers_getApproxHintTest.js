 ...
 */
 ...
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawNECTpromises = activeAccounts.map(account => withdrawNECTfromTrove(account))
  await Promise.all(withdrawNECTpromises)
 ...
 const withdrawNECTfromTrove = async (account) => {
  await borrowerOperations.withdrawNECT(th._100pct, '100000000000000000000', account, account, { from: account })
 }

 // Sequentially add coll and withdraw NECT, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraNECTAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
