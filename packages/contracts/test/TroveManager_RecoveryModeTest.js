...
contract('TroveManager - in Recovery Mode', async accounts => {
  const _1_iBgt = web3.utils.toWei('1', 'ether')
  const _2_iBgt = web3.utils.toWei('2', 'ether')
  const _3_iBgt = web3.utils.toWei('3', 'ether')
  const _3pt5_iBgt = web3.utils.toWei('3.5', 'ether')
  const _6_iBgt = web3.utils.toWei('6', 'ether')
  const _10_iBgt = web3.utils.toWei('10', 'ether')
  const _20_iBgt = web3.utils.toWei('20', 'ether')
  const _21_iBgt = web3.utils.toWei('21', 'ether')
  const _22_iBgt = web3.utils.toWei('22', 'ether')
  const _24_iBgt = web3.utils.toWei('24', 'ether')
  const _25_iBgt = web3.utils.toWei('25', 'ether')
  const _30_iBgt = web3.utils.toWei('30', 'ether')
  ...
  //   // price drops to 1iBGT:150NECT, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  //   await borrowerOperationsAddColl(alice, alice, '1', { from: alice })
  ...
  //   await borrowerOperations.withdrawColl(_1_iBgt, alice, alice, { from: alice })
  ...
  //   await borrowerOperationsAddColl(alice, alice, A_coll, { from: alice })
  ...
  //   //  Alice and Bob withdraw such that the TCR is ~150%
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   // --- TEST ---
  //   // price drops to 1iBGT:100NECT, reducing TCR below 150%, and all Troves below 100% ICR
  //   await priceFeed.setPrice('100000000000000000000')
  ...
  //   //  Alice and Bob withdraw such that the TCR is ~150%
  //   await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   await openTrove({ ICR: toBN(dec(150, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraNECTAmount: spDeposit, extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  //   // --- SETUP ---
  //   //  Bob withdraws up to 2000 NECT of debt, bringing his ICR to 210%
  //   const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  //   // --- TEST ---
  //   // price drops to 1iBGT:100NECT, reducing TCR to 117%
  //   await priceFeed.setPrice('100000000000000000000')
  ...
  //   //  Bob withdraws up to 20000 NECT of debt, bringing his ICR to 210%
  //   const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraNECTAmount: dec(20000, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  //   /*
  //   Prior to Dennis liquidation, total stakes and total collateral were each 27 ibgt. 
  
  //   Check snapshots. Dennis' liquidated collateral is distributed and remains in the system. His 
  //   stake is removed, leaving 24+3*0.995 ibgt total collateral, and 24 ibgt total stakes. */
  ...
  //   /* After Bob's liquidation, Bob's stake (21 ibgt) should be removed from total stakes, 
  ...
  //   // Alice deposits 390NECT to the Stability Pool
  //   await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
  ...
  //   // check pool NECT before liquidation
  //   const stabilityPoolNECT_Before = (await stabilityPool.getTotalNECTDeposits()).toString()
  //   assert.equal(stabilityPoolNECT_Before, '390000000000000000000')
  ...
  //   /* Now, liquidate Bob. Liquidated coll is 21 ibgt, and liquidated debt is 2000 NECT.
    
  //   With 390 NECT in the StabilityPool, 390 NECT should be offset with the pool, leaving 0 in the pool.
  
  //   Stability Pool rewards for alice should be:
  //   NECTLoss: 390NECT
  //   iBGTGain: (390 / 2000) * 21*0.995 = 4.074525 ibgt

  //   After offsetting 390 NECT and 4.074525 ibgt, the remainders - 1610 NECT and 16.820475 ibgt - should be redistributed to all active Troves.
  //  */
  //   // Liquidate Bob
  //   await troveManager.liquidate(bob, { from: owner })

  //   const aliceDeposit = await stabilityPool.getCompoundedNECTDeposit(alice)
  //   const aliceiBGTGain = await stabilityPool.getDepositoriBGTGain(alice)
  //   const aliceExpectediBGTGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt)

  //   assert.equal(aliceDeposit.toString(), 0)
  //   assert.equal(aliceiBGTGain.toString(), aliceExpectediBGTGain)

  //   /* Now, check redistribution to active Troves. Remainders of 1610 NECT and 16.82 ibgt are distributed.
    
  //   Now, only Alice and Dennis have a stake in the system - 3 ibgt each, thus total stakes is 6 ibgt.
  
  //   Rewards-per-unit-staked from the redistribution should be:
  
  //   L_NECTDebt = 1610 / 6 = 268.333 NECT
  //   L_iBGT = 16.820475 /6 =  2.8034125 ibgt
  //   */
  //   const L_NECTDebt = (await troveManager.L_NECTDebt()).toString()
  //   const L_iBGT = (await troveManager.L_iBGT()).toString()

  //   assert.isAtMost(th.getDifference(L_NECTDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
  //   assert.isAtMost(th.getDifference(L_iBGT, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  // })
  ...
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  //   assert.equal(L_NECTDebt, '0')
  //   assert.equal(L_iBGT, '0')
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool NECT is GREATER THAN liquidated debt ---

  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: offsets the trove entirely with the pool", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_totalDebt, extraParams: { from: alice } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: dennis } })

  //   // Alice deposits NECT in the Stability Pool
  ...
  //   /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 NECT, Alice sole depositor.
  //   As liquidated debt (250 NECT) was completely offset

  //   Alice's expected compounded deposit: (1490 - 250) = 1240NECT
  //   Alice's expected iBGT gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ibgt
  
  //   */
  //   const aliceExpectedDeposit = await stabilityPool.getCompoundedNECTDeposit(alice)
  //   const aliceExpectediBGTGain = await stabilityPool.getDepositoriBGTGain(alice)

  //   assert.isAtMost(th.getDifference(aliceExpectedDeposit.toString(), spDeposit.sub(B_totalDebt)), 2000)
  //   assert.isAtMost(th.getDifference(aliceExpectediBGTGain, th.applyLiquidationFee(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))), 3000)
  ...
  //   const bob_balanceBefore = th.toBN(await iBGTToken.balanceOf(bob))
  ...
  //   const bob_balanceAfter = th.toBN(await iBGTToken.balanceOf(bob))
  //   th.assertIsApproximatelyEqual(bob_balanceAfter, bob_expectedBalance.add(th.toBN(bob_remainingCollateral)))
  // })

  // it("liquidate(), with ICR% = 110 < TCR, and StabilityPool NECT > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 220%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_totalDebt, extraParams: { from: alice } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  // it("liquidate(), with  110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: removes stake and updates totalStakes", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
    ...
    // totalStakesSnapshot should have reduced to 22 ibgt - the sum of Alice's coll( 20 ibgt) and Dennis' coll (2 ibgt )
  ...
  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: closes the Trove", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  // it("liquidate(), with 110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: can liquidate troves out of order", async () => {
  //   // taking out 1000 NECT, CR of 200%
  ...
  //   await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: totalLiquidatedDebt, extraParams: { from: whale } })
  ...
  //   // can claim collateral
  //   const dennis_balanceBefore = th.toBN(await iBGTToken.balanceOf(dennis))
  ...
  //   const dennis_balanceAfter = th.toBN(await iBGTToken.balanceOf(dennis))
  ...
  //   const carol_balanceBefore = th.toBN(await iBGTToken.balanceOf(carol))
  ...
  //   const carol_balanceAfter = th.toBN(await iBGTToken.balanceOf(carol))
  ...
  // NECT is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: Trove remains active", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: dennis } })

  //   // Alice deposits 1490 NECT in the Stability Pool
  ...
  //   /* Since the pool only contains 100 NECT, and Bob's pre-liquidation debt was 250 NECT,
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: Trove remains in TroveOwners array", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   // Alice deposits 100 NECT in the Stability Pool
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: nothing happens", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   /*  Since Bob's debt (250 NECT) is larger than all NECT in the Stability Pool, Liquidation won’t happen

  //   After liquidation, totalStakes snapshot should equal Alice's stake (20 ibgt) + Dennis stake (2 ibgt) = 22 ibgt.

  //   Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 22 ibgt.

  //   Bob's new coll and stake should remain the same, and the updated totalStakes should still equal 25 ibgt.
  //   */
  //   const bob_Trove = await troveManager.Troves(bob)
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: updates system shapshots", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   /* After liquidation, totalStakes snapshot should still equal the total stake: 25 ibgt

  //   Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 25 ibgt.*/
  ...
  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: causes correct Pool offset and iBGT gain, and doesn't redistribute to active troves", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  //   assert.equal(aliceExpectediBGTGain.toString(), '0')
  ...
  //   const L_NECTDebt_After = (await troveManager.L_NECTDebt()).toString()
  //   const L_iBGT_After = (await troveManager.L_iBGT()).toString()

  //   assert.equal(L_NECTDebt_After, '0')
  //   assert.equal(L_iBGT_After, '0')
  // })

  // it("liquidate(), with ICR > 110%, and StabilityPool NECT < liquidated debt: ICR of non liquidated trove does not change", async () => {
  //   // --- SETUP ---
  //   // Alice withdraws up to 1500 NECT of debt, and Dennis up to 150, resulting in ICRs of 266%.
  //   // Bob withdraws up to 250 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
  //   // Carol withdraws up to debt of 240 NECT, -> ICR of 250%.
  //   const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(1500, 18), extraParams: { from: alice } })
  //   const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(250, 18), extraParams: { from: bob } })
  //   const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: dec(2000, 18), extraParams: { from: dennis } })
  //   const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraNECTAmount: dec(240, 18), extraParams: { from: carol } })
  ...
  //   //Check SP NECT has been completely emptied
  //   assert.equal((await stabilityPool.getTotalNECTDeposits()).toString(), dec(100, 18))
  ...
  //   await nectToken.transfer(bob, dec(100, 18), { from: alice })
  ...
  //   // Alice provides another 50 NECT to pool
  ...
  //   //Check SP NECT has been completely emptied
  //   assert.equal((await stabilityPool.getTotalNECTDeposits()).toString(), dec(150, 18))
  ...
  // it("liquidate() with ICR > 110%, and StabilityPool NECT < liquidated debt: total liquidated coll and debt is correct", async () => {
  //   // Whale provides 50 NECT to the SP
  //   await openTrove({ ICR: toBN(dec(300, 16)), extraNECTAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  //   // Alice creates a single trove with 0.62 iBGT and a debt of 62 NECT, and provides 10 NECT to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 NECT in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or iBGT gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraNECTAmount: spDeposit, extraParams: { from: bob } })

    // Bob sends tokens to Dennis, who has no trove
    await nectToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides 200 NECT to SP
    ...
    assert.isAtMost(th.getDifference(dennis_iBGTGain_Before, th.applyLiquidationFee(C_coll)), 1000)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraNECTAmount: dec(1000, 18), extraParams: { from: whale } })

    const { nectAmount: A_nectAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(300, 18), extraParams: { from: alice } })
    const { nectAmount: B_nectAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(200, 18), extraParams: { from: bob } })
    const { nectAmount: C_nectAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraNECTAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 NECT of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 NECT of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 NECT, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 iBGT
    await priceFeed.setPrice('200000000000000000000')
    const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraNECTAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_netDebt_2, extraParams: { from: dennis } })
    ...
    // can claim collateral
    const bob_balanceBefore_2 = th.toBN(await iBGTToken.balanceOf(bob))
    ...
    const bob_balanceAfter_2 = th.toBN(await iBGTToken.balanceOf(bob))
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 NECT of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraNECTAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 NECT of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_netDebt, extraParams: { from: dennis } })

    // --- TEST ---
    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 iBGT
    ...
    // Bob re-opens the trove, price 200, total debt 250 NECT, ICR = 240% (lowest one)
    const { collateral: B_coll_2, totalDebt: B_totalDebt_2 } = await openTrove({ ICR: toBN(dec(240, 16)), extraParams: { from: bob, value: _3_iBgt } })
    // Alice deposits NECT in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraNECTAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some NECT from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraNECTAmount: dec(5000, 18), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraNECTAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits NECT to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1iBGT:90NECT, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1iBGT:85NECT, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraNECTAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraNECTAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraNECTAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraNECTAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 NECT
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraNECTAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraNECTAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraNECTAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraNECTAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraNECTAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 NECT in Stability Pool
    assert.equal((await stabilityPool.getTotalNECTDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds NECT to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm NECT in Stability Pool
    assert.equal((await stabilityPool.getTotalNECTDeposits()).toString(), spDeposit.toString())
    ...
    // can claim collateral
    const freddy_balanceBefore = th.toBN(await iBGTToken.balanceOf(freddy))
    ...
    const freddy_balanceAfter = th.toBN(await iBGTToken.balanceOf(freddy))
    th.assertIsApproximatelyEqual(freddy_balanceAfter, freddy_expectedBalance.add(th.toBN(freddy_remainingCollateral)))

    const greta_balanceBefore = th.toBN(await iBGTToken.balanceOf(greta))
    ...
    const greta_balanceAfter = th.toBN(await iBGTToken.balanceOf(greta))
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingiBGTReward(alice))
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingNECTDebtReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { nectAmount: nectAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { nectAmount: nectAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { nectAmount: nectAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await nectToken.balanceOf(dennis)).toString(), nectAmountD)
    assert.equal((await nectToken.balanceOf(erin)).toString(), nectAmountE)
    assert.equal((await nectToken.balanceOf(freddy)).toString(), nectAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and iBGT gain", async () => {
    // Whale provides NECT to the SP
    const { nectAmount: W_nectAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraNECTAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_nectAmount, ZERO_ADDRESS, { from: whale })

    const { nectAmount: A_nectAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraNECTAmount: dec(40, 18), extraParams: { from: alice } })
    const { nectAmount: B_nectAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_nectAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_nectAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_nectAmount.add(A_nectAmount).add(B_nectAmount)
    ...
    // Check NECT in Pool
    assert.equal((await stabilityPool.getTotalNECTDeposits()).toString(), totalDeposit)
    ...
    Total NECT in Pool: 680 NECT

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 NECT
    Total liquidated iBGT = 1 + 3 + 1 = 5 iBGT

    Whale NECT Loss: 500 * (400/680) = 294.12 NECT
    Alice NECT Loss:  500 *(40/680) = 29.41 NECT
    Bob NECT Loss: 500 * (240/680) = 176.47 NECT

    Whale remaining deposit: (400 - 294.12) = 105.88 NECT
    Alice remaining deposit: (40 - 29.41) = 10.59 NECT
    Bob remaining deposit: (240 - 176.47) = 63.53 NECT

    Whale iBGT Gain: 5*0.995 * (400/680) = 2.93 iBGT
    Alice iBGT Gain: 5*0.995 *(40/680) = 0.293 iBGT
    Bob iBGT Gain: 5*0.995 * (240/680) = 1.76 iBGT

    Total remaining deposits: 180 NECT
    Total iBGT gain: 5*0.995 iBGT */
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedNECTDeposit(whale)).toString()
    ...
    const whale_iBGTGain = (await stabilityPool.getDepositoriBGTGain(whale)).toString()
    const alice_iBGTGain = (await stabilityPool.getDepositoriBGTGain(alice)).toString()
    const bob_iBGTGain = (await stabilityPool.getDepositoriBGTGain(bob)).toString()

    const liquidatedDebt = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    const liquidatedColl = A_coll.add(B_coll).add(C_coll)
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_nectAmount.sub(liquidatedDebt.mul(W_nectAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_nectAmount.sub(liquidatedDebt.mul(A_nectAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_nectAmount.sub(liquidatedDebt.mul(B_nectAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_iBGTGain, th.applyLiquidationFee(liquidatedColl).mul(W_nectAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(alice_iBGTGain, th.applyLiquidationFee(liquidatedColl).mul(A_nectAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(bob_iBGTGain, th.applyLiquidationFee(liquidatedColl).mul(B_nectAmount).div(totalDeposit)), 2000)
    ...
    assert.isAtMost(th.getDifference(total_NECTinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_iBGTinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or iBGT gain", async () => {
    // Whale provides 400 NECT to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraNECTAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraNECTAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraNECTAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check NECT and iBGT in Pool  before
    ...
    assert.equal(NECTinSP_Before, dec(800, 18))
    assert.equal(iBGTinSP_Before, '0')
    ...
    // Check NECT and iBGT in Pool after
    ...
    const whale_iBGTGain_After = (await stabilityPool.getDepositoriBGTGain(whale)).toString()
    ...
    assert.equal(whale_iBGTGain_After, '0')
    assert.equal(alice_iBGTGain_After, '0')
    assert.equal(bob_iBGTGain_After, '0')
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 NECT in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides NECT to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 NECT in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 NECT in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 NECT and system coll 2.3 iBGT
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides NECT to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 NECT in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, nectGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(nectGasComp.toString(), dec(400, 18))
    ...
    // can claim collateral
    const alice_balanceBefore = th.toBN(await iBGTToken.balanceOf(alice))
    const ALICE_GAS = th.gasUsed(await borrowerOperations.claimCollateral({ from: alice, gasPrice: GAS_PRICE  }))
    const alice_balanceAfter = th.toBN(await iBGTToken.balanceOf(alice))
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraNECTAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraNECTAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraNECTAmount: A_totalDebt, extraParams: { from: whale } })

    // Alice deposits NECT to Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })

    // to compensate borrowing fee
    await nectToken.transfer(alice, A_totalDebt, { from: whale })
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 NECT and system coll by 2 iBGT
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool NECT > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraNECTAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraNECTAmount: dec(500, 18), extraParams: { from: freddy } })

    // Whale provides 1000 NECT to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingNECTDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingNECTDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingNECTDebtReward(carol))

    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingiBGTReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingiBGTReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingiBGTReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 NECT
    ...
    // Expected Stability Pool to fully absorb B (92 NECT + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalNECTDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalNECTDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 NECT to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraNECTAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
