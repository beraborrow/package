...
contract('StabilityPool Scale Factor issue tests', async accounts => {
  ...
  const getNECTAmountForDesiredDebt = async (desiredDebt) => (await getOpenTroveNECTAmount(dec(desiredDebt, 18))).add(th.toBN(1))
  ...
  it("1. Liquidation succeeds after P reduced to 1", async () => {
    ...
    const deposit_1 = deposit_0.sub(await stabilityPool.getTotalNECTDeposits())
    ...
    const deposit_2 = deposit_0.sub(await stabilityPool.getTotalNECTDeposits())
  ...
  it("2. New deposits can be made after P reduced to 1", async () => {
    ...
    for (let i=0; i < 3; i ++) {
      await nectToken.transfer(newDepositors[i], newDeposits[i], {from: whale})
      await stabilityPool.provideToSP(newDeposits[i], frontEnds[i], {from: newDepositors[i]})
      assert.isTrue((await stabilityPool.getCompoundedNECTDeposit(newDepositors[i])).eq(newDeposits[i]))
    }
  })
  ...
  it("3. Liquidation succeeds when P == 1 and liquidation has newProductFactor == 1e9", async () => {
    ...
    const deposit_2 = deposit_0.sub(await stabilityPool.getTotalNECTDeposits()).add(th.toBN(dec(2,12)))
    await stabilityPool.provideToSP(deposit_2, ZERO_ADDRESS, {from: A})
  ...
  it("4. Liquidation succeeds when P == 1 and liquidation has newProductFactor > 1e9", async () => {
    ...
    const deposit_2 = deposit_0.mul(th.toBN(2)).sub(await stabilityPool.getTotalNECTDeposits())
    await stabilityPool.provideToSP(deposit_2, ZERO_ADDRESS, {from: A})
  ...
  it("5. Depositor have correct depleted stake after deposit at P == 1 and scale changing liq (with newProductFactor == 1e9)", async () => {
    ...
    // D makes deposit of 1000 NECT
    const D_deposit = dec(1, 21)
    await nectToken.transfer(D, dec(1, 21), {from: whale})
    ...
    const D_depletedDeposit = await stabilityPool.getCompoundedNECTDeposit(D)
