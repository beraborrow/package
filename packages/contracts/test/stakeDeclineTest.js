...
contract('TroveManager', async accounts => {
  ...
  it("A given trove's stake decline is negligible with adjustments and tiny liquidations", async () => {
    ...
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(1, 31)), ZERO_ADDRESS, ZERO_ADDRESS, dec(2, 29), { from: A })
    
    // Make 5 large troves B, C, D, E, F at ~10% total collateral
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(2, 30)), ZERO_ADDRESS, ZERO_ADDRESS, dec(4, 28), { from: B })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(2, 30)), ZERO_ADDRESS, ZERO_ADDRESS, dec(4, 28), { from: C })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(2, 30)), ZERO_ADDRESS, ZERO_ADDRESS, dec(4, 28), { from: D })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(2, 30)), ZERO_ADDRESS, ZERO_ADDRESS, dec(4, 28), { from: E })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(2, 30)), ZERO_ADDRESS, ZERO_ADDRESS, dec(4, 28), { from: F })
    ...
    for (account of tinyTroves) {
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveNECTAmount(dec(1, 22)), ZERO_ADDRESS, ZERO_ADDRESS, dec(2, 20), { from: account })
    }
