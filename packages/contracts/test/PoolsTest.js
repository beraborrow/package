...
contract('ActivePool', async accounts => {
  ...
  it('sendiBGT(): decreases the recorded iBGT balance by the correct amount', async () => {
    ...
    const activePool_initialBalance = web3.utils.toBN(await iBGTToken.balanceOf(activePool.address))
    assert.equal(activePool_initialBalance, 0)
    // start pool with 2 ibgt
    //await web3.eth.sendTransaction({ from: mockBorrowerOperationsAddress, to: activePool.address, value: dec(2, 'ether') })
      await iBGTToken.increaseAllowance(mockBorrowerOperations.address, activePool.address, dec(2, 'ether'))
      await iBGTToken.mint(mockBorrowerOperations.address, dec(2, 'ether'))

      const tx1 = await mockBorrowerOperations.forward(activePool.address, '0x', { from: owner })
    assert.isTrue(tx1.receipt.status)

    const activePool_BalanceBeforeTx = web3.utils.toBN(await iBGTToken.balanceOf(activePool.address))
    const alice_Balance_BeforeTx = web3.utils.toBN(await iBGTToken.balanceOf(alice))
    ...
    const activePool_BalanceAfterTx = web3.utils.toBN(await iBGTToken.balanceOf(activePool.address))
    const alice_Balance_AfterTx = web3.utils.toBN(await iBGTToken.balanceOf(alice))
...
contract('DefaultPool', async accounts => {
  ...
  it('sendiBGTToActivePool(): decreases the recorded iBGT balance by the correct amount', async () => {
    // setup: give pool 2 ibgt
    const defaultPool_initialBalance = web3.utils.toBN(await iBGTToken.balanceOf(defaultPool.address))
    console.log (defaultPool_initialBalance.toString(), ">>>>>>>>>>>")
    ...
    const defaultPool_BalanceBeforeTx = web3.utils.toBN(await iBGTToken.balanceOf(defaultPool.address))
    const activePool_Balance_BeforeTx = web3.utils.toBN(await iBGTToken.balanceOf(mockActivePool.address))
    ...
    const defaultPool_BalanceAfterTx = web3.utils.toBN(await iBGTToken.balanceOf(defaultPool.address))
    const activePool_Balance_AfterTx = web3.utils.toBN(await iBGTToken.balanceOf(mockActivePool.address))
