...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const alice_Coll = ((await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingiBGTReward(alice)))
      .toString()
    const carol_Coll = ((await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingiBGTReward(carol)))
      .toString()

    /* Expected collateral:
    A: Alice receives 0.995 iBGT from L1, and ~3/5*0.995 iBGT from L2.
    expect aliceColl = 2 + 0.995 + 2.995/4.995 * 0.995 = 3.5916 iBGT

    C: Carol receives ~2/5 iBGT from L2
    expect carolColl = 2 + 2/4.995 * 0.995 = 2.398 iBGT

    Total coll = 4 + 2 * 0.995 iBGT
    */
    ...
    const entireSystemColl = (await activePool.getiBGT()).add(await defaultPool.getiBGT()).toString()
    assert.equal(entireSystemColl, A_coll.add(C_coll).add(th.applyLiquidationFee(B_coll.add(D_coll))))

    // check NECT gas compensation
    assert.equal((await nectToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingiBGTReward(bob)))
      .toString()
    const dennis_Coll = ((await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingiBGTReward(dennis)))
      .toString()
    const erin_Coll = ((await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingiBGTReward(erin)))
      .toString()

    /* Expected collateral:
    A and B receives 1/2 iBGT * 0.995 from L1.
    total Coll: 3

    A, B, receive (2.4975)/8.995 * 0.995 iBGT from L2.
    
    D, E receive 2/8.995 * 0.995 iBGT from L2.

    expect A, B coll  = 2 +  0.4975 + 0.2763  =  iBGT
    expect D, E coll  = 2 + 0.2212  =  iBGT
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has iBGT from all previously liquidated troves", async () => {
    // A, B  open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: bob } })
    ...
    const freddy_iBGTReward = (await troveManager.getPendingiBGTReward(freddy)).toString()

    /* Expected collateral:
     A-E should have been liquidated
     trove F should have acquired all iBGT in the system: 1 iBGT initial coll, and 0.995^5+0.995^4+0.995^3+0.995^2+0.995 from rewards = 5.925 iBGT
    */
    ...
    const gainediBGT = th.applyLiquidationFee(
    ...
    assert.isAtMost(th.getDifference(freddy_iBGTReward, gainediBGT), 1000)

    const entireSystemColl = (await activePool.getiBGT()).add(await defaultPool.getiBGT()).toString()
    assert.isAtMost(th.getDifference(entireSystemColl, F_coll.add(gainediBGT)), 1000)

    // check NECT gas compensation
    assert.equal((await nectToken.balanceOf(owner)).toString(), dec(1000, 18))
  })
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraNECTAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100000, 18), extraParams: { from: E } })
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    // Check accumulated iBGT gain for each trove
    const B_iBGTGain_1 = await troveManager.getPendingiBGTReward(B)
    const C_iBGTGain_1 = await troveManager.getPendingiBGTReward(C)
    const D_iBGTGain_1 = await troveManager.getPendingiBGTReward(D)
    const E_iBGTGain_1 = await troveManager.getPendingiBGTReward(E)

    // Check gains are what we'd expect from a distribution proportional to each trove's entire coll
    const B_expectedPendingiBGT_1 = A_collRedistribution.mul(B_entireColl_0).div(denominatorColl_1)
    const C_expectedPendingiBGT_1 = A_collRedistribution.mul(C_entireColl_0).div(denominatorColl_1)
    const D_expectedPendingiBGT_1 = A_collRedistribution.mul(D_entireColl_0).div(denominatorColl_1)
    const E_expectedPendingiBGT_1 = A_collRedistribution.mul(E_entireColl_0).div(denominatorColl_1)

    assert.isAtMost(getDifference(B_expectedPendingiBGT_1, B_iBGTGain_1), 1e8)
    assert.isAtMost(getDifference(C_expectedPendingiBGT_1, C_iBGTGain_1), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingiBGT_1, D_iBGTGain_1), 1e8)
    ...
    const B_iBGTGain_2 = await troveManager.getPendingiBGTReward(B)
    const D_iBGTGain_2 = await troveManager.getPendingiBGTReward(D)
    const E_iBGTGain_2 = await troveManager.getPendingiBGTReward(E)

    // Since B topped up, he has no previous pending iBGT gain
    const B_expectedPendingiBGT_2 = C_collRedistribution.mul(B_entireColl_1).div(denominatorColl_2)

    // D & E's accumulated pending iBGT gain includes their previous gain
    const D_expectedPendingiBGT_2 = C_collRedistribution.mul(D_entireColl_1).div(denominatorColl_2)
      .add(D_expectedPendingiBGT_1)

    const E_expectedPendingiBGT_2 = C_collRedistribution.mul(E_entireColl_1).div(denominatorColl_2)
      .add(E_expectedPendingiBGT_1)

    assert.isAtMost(getDifference(B_expectedPendingiBGT_2, B_iBGTGain_2), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingiBGT_2, D_iBGTGain_2), 1e8)
    ...
    const B_iBGTGain_3 = await troveManager.getPendingiBGTReward(B)
    const D_iBGTGain_3 = await troveManager.getPendingiBGTReward(D)

    // Since B topped up, he has no previous pending iBGT gain
    const B_expectedPendingiBGT_3 = E_collRedistribution.mul(B_entireColl_2).div(denominatorColl_3)

    // D'S accumulated pending iBGT gain includes their previous gain
    const D_expectedPendingiBGT_3 = E_collRedistribution.mul(D_entireColl_2).div(denominatorColl_3)
      .add(D_expectedPendingiBGT_2)

    assert.isAtMost(getDifference(B_expectedPendingiBGT_3, B_iBGTGain_3), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingiBGT_3, D_iBGTGain_3), 1e8)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    // Expect Bob now holds all iBgt and NECTDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 iBgt and 110*3 NECT (10 each for gas compensation)
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingiBGTReward(bob)))
      .toString()

    const bob_NECTDebt = ((await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingNECTDebtReward(bob)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_NECTDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 iBGT, 55 NECT
     L2: (2.4975/3.995)*0.995 = 0.622 iBGT , 110*(2.4975/3.995)= 68.77 NECTDebt

    coll: 3.1195 iBGT
    debt: 233.77 NECTDebt

     Alice rewards:
    L1 1/2*0.995 iBGT, 55 NECT
    L2 (1.4975/3.995)*0.995 = 0.3730 iBGT, 110*(1.4975/3.995) = 41.23 NECTDebt

    coll: 1.8705 iBGT
    debt: 146.23 NECTDebt

    totalColl: 4.99 iBGT
    totalDebt 380 NECT (includes 50 each for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingiBGTReward(bob)))
      .toString()
    ...
    const alice_NECTDebt = ((await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingNECTDebtReward(alice)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_NECTDebt, expected_B_debt), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_NECTDebt, expected_A_debt), 10000)
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    const _998_iBgt = toBN('998000000000000000000')
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ extraNECTAmount: dec(110, 18), extraParams: { from: carol, value: _998_iBgt } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: dennis, value: dec(1000, 'ether') } })
    ...
    // Expected rewards:  alice: 1 iBGT, bob: 1 iBGT, carol: 998 iBGT
    const alice_iBGTReward_1 = await troveManager.getPendingiBGTReward(alice)
    const bob_iBGTReward_1 = await troveManager.getPendingiBGTReward(bob)
    const carol_iBGTReward_1 = await troveManager.getPendingiBGTReward(carol)

    //Expect 1000 + 1000*0.995 iBGT in system now
    const entireSystemColl_1 = (await activePool.getiBGT()).add(await defaultPool.getiBGT()).toString()
    ...
    th.assertIsApproximatelyEqual(alice_iBGTReward_1.toString(), th.applyLiquidationFee(D_coll).mul(A_coll).div(totalColl))
    th.assertIsApproximatelyEqual(bob_iBGTReward_1.toString(), th.applyLiquidationFee(D_coll).mul(B_coll).div(totalColl))
    th.assertIsApproximatelyEqual(carol_iBGTReward_1.toString(), th.applyLiquidationFee(D_coll).mul(C_coll).div(totalColl))
    ...
    // E opens with another 1996 iBGT
    ...
    /* Expected iBGT rewards: 
     Carol = 1992.01/1996 * 1996*0.995 = 1982.05 iBGT
     Alice = 1.995/1996 * 1996*0.995 = 1.985025 iBGT
     Bob = 1.995/1996 * 1996*0.995 = 1.985025 iBGT

    therefore, expected total collateral:

    Carol = 1991.01 + 1991.01 = 3974.06
    Alice = 1.995 + 1.985025 = 3.980025 iBGT
    Bob = 1.995 + 1.985025 = 3.980025 iBGT
    ...
    */
    ...
    //Expect 3982.02 iBGT in system now
    const entireSystemColl_3 = (await activePool.getiBGT()).add(await defaultPool.getiBGT()).toString()
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))

    // check NECT gas compensation
    th.assertIsApproximatelyEqual((await nectToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // Expected rewards:  alice: 1 iBGT, bob: 1 iBGT, carol: 998 iBGT (*0.995)
    const alice_iBGTReward_1 = await troveManager.getPendingiBGTReward(alice)
    const bob_iBGTReward_1 = await troveManager.getPendingiBGTReward(bob)
    const carol_iBGTReward_1 = await troveManager.getPendingiBGTReward(carol)

    //Expect 1995 iBGT in system now
    const entireSystemColl_1 = (await activePool.getiBGT()).add(await defaultPool.getiBGT()).toString()
    assert.equal(entireSystemColl_1, A_coll.add(B_coll).add(C_coll).add(th.applyLiquidationFee(D_coll)))
    ...
    /* Alice, Bob, Carol each adds 1 iBGT to their troves, 
    ...
    // E opens with another 1998 iBGT
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected iBGT rewards: 
     Carol = 1992.01/1998 * 1998*0.995 = 1982.04995 iBGT
     Alice = 2.995/1998 * 1998*0.995 = 2.980025 iBGT
     Bob = 2.995/1998 * 1998*0.995 = 2.980025 iBGT

    therefore, expected total collateral:

    Carol = 1992.01 + 1982.04995 = 3974.05995
    Alice = 2.995 + 2.980025 = 5.975025 iBGT
    Bob = 2.995 + 2.980025 = 5.975025 iBGT
    ...
    */
    ...
    //Expect 3986.01 iBGT in system now
    const entireSystemColl_3 = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    //Bob withdraws 0.5 iBGT from his trove
    ...
    // Expect Bob now holds all iBgt and NECTDebt in the system: 2.5 iBgt and 300 NECT
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 iBGT, 55 NECT
     L2: (0.9975/2.495)*0.995 = 0.3978 iBGT , 110*(0.9975/2.495)= 43.98 NECTDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 iBGT
    debt: (110 + 55 + 43.98 = 208.98 NECTDebt 

     Alice rewards:
    L1 0.4975, 55 NECT
    L2 (1.4975/2.495)*0.995 = 0.5972 iBGT, 110*(1.4975/2.495) = 66.022 NECTDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 iBGT
    debt: (50 + 55 + 66.022) = 171.022 NECT Debt

    totalColl: 3.49 iBGT
    totalDebt 380 NECT (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingiBGTReward(bob)))
      .toString()
    ...
    const entireSystemColl = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    ...
    const entireSystemDebt = (await activePool.getNECTDebt()).add(await defaultPool.getNECTDebt())
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    //Expect 1995 iBGT in system now
    const entireSystemColl_1 = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    ...
    /* Expected iBGT rewards: 
     Carol = 1990.01/1994 * 1994*0.995 = 1980.05995 iBGT
     Alice = 1.995/1994 * 1994*0.995 = 1.985025 iBGT
     Bob = 1.995/1994 * 1994*0.995 = 1.985025 iBGT
    ...
    */
    ...
    //Expect 3978.03 iBGT in system now
    const entireSystemColl_3 = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    /* Alice, Bob, Carol each withdraw 0.5 iBGT to their troves, 
    ...
    //Expect 1993.5 iBGT in system now
    const entireSystemColl_2 = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    ...
    // E opens with another 1993.5 iBGT
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected iBGT rewards: 
     Carol = 1990.51/1993.5 * 1993.5*0.995 = 1980.55745 iBGT
     Alice = 1.495/1993.5 * 1993.5*0.995 = 1.487525 iBGT
     Bob = 1.495/1993.5 * 1993.5*0.995 = 1.487525 iBGT

    therefore, expected total collateral:

    Carol = 1990.51 + 1980.55745 = 3971.06745
    Alice = 1.495 + 1.487525 = 2.982525 iBGT
    Bob = 1.495 + 1.487525 = 2.982525 iBGT

    total = 3977.0325 iBGT
    */
    ...
    //Expect 3977.0325 iBGT in system now
    const entireSystemColl_3 = (await activePool.getiBGT()).add(await defaultPool.getiBGT())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraNECTAmount: dec(110, 18), extraParams: { from: freddy } })
    ...
    const carol_pendingiBGTReward = (await troveManager.getPendingiBGTReward(carol)).toString()

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString()
    const dennis_pendingiBGTReward = (await troveManager.getPendingiBGTReward(dennis)).toString()

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString()
    const erin_pendingiBGTReward = (await troveManager.getPendingiBGTReward(erin)).toString()
    ...
    // Check pending iBGT rewards of C, D, E
    assert.isAtMost(th.getDifference(carol_pendingiBGTReward, C_collAfterL3.sub(C_collAfterL1)), 1000000)
    assert.isAtMost(th.getDifference(dennis_pendingiBGTReward, D_collAfterL3.sub(D_collAfterL2)), 1000000)
    assert.isAtMost(th.getDifference(erin_pendingiBGTReward, E_collAfterL3.sub(E_coll)), 1000000)

    // Check systemic collateral
    const activeColl = (await activePool.getiBGT()).toString()
    const defaultColl = (await defaultPool.getiBGT()).toString()
    ...
    // check NECT gas compensation
    assert.equal((await nectToken.balanceOf(owner)).toString(), dec(600, 18))
  })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    // D opens trove: 0.035 iBGT
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraNECTAmount: dec(100, 18), extraParams: { from: dennis, value: toBN(dec(35, 15)) } })
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingiBGTReward(dennis), D_pendingRewardsAfterL2), 10000000)
    ...
    F: 0.0007 iBGT
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraNECTAmount: dec(100, 18), extraParams: { from: erin, value: toBN(dec(1, 22)) } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraNECTAmount: dec(100, 18), extraParams: { from: freddy, value: toBN('700000000000000') } })
