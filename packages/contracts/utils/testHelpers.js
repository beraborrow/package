...
class TestHelper {
  ...
  // Adds the gas compensation (50 NECT)
  ...
  /*
   * given the requested NECT amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, nectAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(nectAmount)
    const compositeDebt = await this.getCompositeDebt(contracts, nectAmount)
    return compositeDebt.add(fee)
  }

  /*
   * given the desired total debt, returns the NECT amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveNECTAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, nectAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(nectAmount)
    return nectAmount.add(fee)
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const NECTAmount = redemptionTx.logs[i].args[0]
        const totalNECTRedeemed = redemptionTx.logs[i].args[1]
        const totaliBGTDrawn = redemptionTx.logs[i].args[2]
        const iBGTFee = redemptionTx.logs[i].args[3]

        return [NECTAmount, totalNECTRedeemed, totaliBGTDrawn, iBGTFee]
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const nectGasComp = liquidationTx.logs[i].args[3]

        return [liquidatedDebt, liquidatedColl, collGasComp, nectGasComp]
      }
    }
  ...
  static getNECTFeeFromNECTBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "NECTBorrowingFeePaid") {
        return (tx.logs[i].args[1]).toString()
      }
    }
    throw ("The transaction logs do not contain an NECTBorrowingFeePaid event")
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingiBGTReward = await contracts.troveManager.getPendingiBGTReward(account)
    const pendingNECTDebtReward = await contracts.troveManager.getPendingNECTDebtReward(account)
    const entireColl = rawColl.add(pendingiBGTReward)
    const entireDebt = rawDebt.add(pendingNECTDebtReward)
  ...
  static async getCollAndDebtFromWithdrawNECT(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayNECT(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, iBGTChange, NECTChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = NECTChange.gt(this.toBN('0')) ? await contracts.troveManager.getBorrowingFee(NECTChange) : this.toBN('0')
    const newColl = entireColl.add(iBGTChange)
    const newDebt = entireDebt.add(NECTChange).add(fee)
  ...
  static async openTrove_allAccounts(accounts, contracts, iBGTAmount, NECTAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, NECTAmount)
  ...
  static async openTrove_allAccounts_randomiBGT(miniBGT, maxiBGT, accounts, contracts, NECTAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, NECTAmount)
  ...
  static async openTrove_allAccounts_randomiBGT_ProportionalNECT(miniBGT, maxiBGT, accounts, contracts, proportion) {
    const gasCostList = []
  
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(miniBGT, maxiBGT)
      const proportionalNECT = (web3.utils.toBN(proportion)).mul(web3.utils.toBN(randCollAmount))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalNECT)
  ...
  static async openTrove_allAccounts_randomiBGT_randomNECT(miniBGT, maxiBGT, accounts, contracts, minNECTProportion, maxNECTProportion, logging = false) {
    const gasCostList = []
    const price = await contracts.priceFeedTestnet.getPrice()
    ...
    for (const account of accounts) {
      ...
      const randNECTProportion = this.randAmountInWei(minNECTProportion, maxNECTProportion)
      const proportionalNECT = (web3.utils.toBN(randNECTProportion)).mul(web3.utils.toBN(randCollAmount).div(_1e18))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalNECT)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)
      ...
      if (logging && tx.receipt.status) {
        i++
        const ICR = await contracts.troveManager.getCurrentICR(account, price)
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalNECT} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
  ...
  static async openTrove_allAccounts_randomNECT(minNECT, maxNECT, accounts, contracts, iBGTAmount) {
    const gasCostList = []

    for (const account of accounts) {
      const randNECTAmount = this.randAmountInWei(minNECT, maxNECT)
  ...
  static async openTrove_allAccounts_decreasingNECTAmounts(accounts, contracts, iBGTAmount, maxNECTAmount) {
    const gasCostList = []

    let i = 0
    for (const account of accounts) {
      const NECTAmount = (maxNECTAmount - i).toString()
      const NECTAmountWei = web3.utils.toWei(NECTAmount, 'ether')
  ...
  static async openTrove(contracts, {
    ...
    if (!extraNECTAmount) extraNECTAmount = this.toBN(0)
    else if (typeof extraNECTAmount == 'string') extraNECTAmount = this.toBN(extraNECTAmount)
    ...
    const nectAmount = MIN_DEBT.add(extraNECTAmount)
  ...
  static async withdrawNECT(contracts, {
    maxFeePercentage,
    nectAmount,
    ICR,
    upperHint,
    ...
    assert(!(nectAmount && ICR) && (nectAmount || ICR), "Specify either nect amount or target ICR, but not both")
    ...
    if (ICR) {
      ...
      nectAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, nectAmount)
    }

    await contracts.borrowerOperations.withdrawNECT(maxFeePercentage, nectAmount, upperHint, lowerHint, extraParams)
  ...
  static async adjustTrove_allAccounts(accounts, contracts, iBGTAmount, NECTAmount) {
    const gasCostList = []

    for (const account of accounts) {
      let tx;

      let iBGTChangeBN = this.toBN(iBGTAmount)
      let NECTChangeBN = this.toBN(NECTAmount)

      const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, iBGTChangeBN, NECTChangeBN)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const zero = this.toBN('0')

      let isDebtIncrease = NECTChangeBN.gt(zero)
      NECTChangeBN = NECTChangeBN.abs() 

      // Add iBGT to trove
      if (iBGTChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, NECTChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: iBGTChangeBN })
      // Withdraw iBGT from trove
      } else if (iBGTChangeBN.lt(zero)) {
        iBGTChangeBN = iBGTChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, iBGTChangeBN, NECTChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
      }
  ...
  static async adjustTrove_allAccounts_randomAmount(accounts, contracts, iBGTMin, iBGTMax, NECTMin, NECTMax) {
    const gasCostList = []

    for (const account of accounts) {
      let tx;
  
      let iBGTChangeBN = this.toBN(this.randAmountInWei(iBGTMin, iBGTMax))
      let NECTChangeBN = this.toBN(this.randAmountInWei(NECTMin, NECTMax))
      ...
      // Withdraw iBGT from trove
        ...
        iBGTChangeBN = iBGTChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, iBGTChangeBN, NECTChangeBN, isDebtIncrease, lowerHint,  upperHint,{ from: account })
      }

      const gas = this.gasUsed(tx)
      // console.log(`iBGT change: ${iBGTChangeBN},  NECTChange: ${NECTChangeBN}, gas: ${gas} `)
  ...
  static async withdrawNECT_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawNECT(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawNECT(this._100pct, amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawNECT_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randNECTAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawNECT(contracts, account, randNECTAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawNECT(this._100pct, randNECTAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayNECT_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayNECT(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayNECT(amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayNECT_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randNECTAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromRepayNECT(contracts, account, randNECTAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayNECT(randNECTAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async redeemCollateral(redeemer, contracts, NECTAmount, gasPrice = 0, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, NECTAmount, maxFee, gasPrice)
    const gas = await this.gasUsed(tx)
    return gas
  }

  static async redeemCollateralAndGetTxObject(redeemer, contracts, NECTAmount, gasPrice, maxFee = this._100pct) {
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randNECTAmount)
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async performRedemptionTx(redeemer, price, contracts, NECTAmount, maxFee = 0, gasPrice_toUse = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(NECTAmount, price, gasPrice_toUse)
    ...
    const tx = await contracts.troveManager.redeemCollateral(NECTAmount,
      firstRedemptionHint,
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = []
    for (const account of accounts) {
      const randomNECTAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.provideToSP(randomNECTAmount, this.ZERO_ADDRESS, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomNECTAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.withdrawFromSP(randomNECTAmount, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawiBGTGainToTrove_allAccounts(accounts, contracts) {
    const gasCostList = []
    for (const account of accounts) {
      ...
      console.log(`entireColl: ${entireColl}`)
      console.log(`entireDebt: ${entireDebt}`)
      const iBGTGain = await contracts.stabilityPool.getDepositoriBGTGain(account)
      const newColl = entireColl.add(iBGTGain)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, entireDebt)

      const tx = await contracts.stabilityPool.withdrawiBGTGainToTrove(upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  // --- POLLEN & Lockup Contract functions ---
  ...
  static async getTimeFromSystemDeployment(pollenToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await pollenToken.getDeploymentStartTime()
