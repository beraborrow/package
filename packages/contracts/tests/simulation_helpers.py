...
Ether is the collateral for NECT. The ether price $P_t^e$ follows 
...
> (collateral in ether, debt in NECT, collateral ratio target, rational inattention, collateral ratio)
...
print(" - iBGT period 1 -")
print(f"Min iBGT price: {min(price_ether[1:period1])}")
print(f"Max iBGT price: {max(price_ether[1:period1])}")
for i in range(period1, period2):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether2))
print(" - iBGT period 2 -")
print(f"Min iBGT price: {min(price_ether[period1:period2])}")
print(f"Max iBGT price: {max(price_ether[period1:period2])}")
for i in range(period2, period3):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether3))
print(" - iBGT period 3 -")
print(f"Min iBGT price: {min(price_ether[period2:period3])}")
print(f"Max iBGT price: {max(price_ether[period2:period3])}")
for i in range(period3, period4):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether4))
print(" - iBGT period 4 -")
print(f"Min iBGT price: {min(price_ether[period3:period4])}")
print(f"Max iBGT price: {max(price_ether[period3:period4])}")
...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_NECT, price_POLLEN_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalNECTDeposits() / 1e18
    stability_pool_eth_previous = contracts.stabilityPool.getiBGT() / 1e18
    ...
    stability_pool_eth_current = contracts.stabilityPool.getiBGT() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_NECT
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_NECT, index):
    if len(active_accounts) == 0:
        return [0]
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_NECT, index):
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    ...
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedNECTamount] = contracts.hintHelpers.getRedemptionHints(nect_balance, price_ether_current, 70)
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_NECT, index):
