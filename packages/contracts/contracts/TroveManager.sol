...
contract TroveManager is BeraBorrowBase, Ownable, CheckContract, ITroveManager {
    ...
    INECTToken public override nectToken;

    IPOLLENToken public override pollenToken;

    IPOLLENStaking public override pollenStaking;
    ...
    */
    ...
    // The timestamp of the latest fee operation (redemption or new NECT issuance)
    ...
    **/
    ...
    event POLLENTokenAddressChanged(address _pollenTokenAddress);
        ...
        onlyOwner
    {
        ...
        emit POLLENTokenAddressChanged(_pollenTokenAddress);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        singleLiquidation.NECTGasCompensation = NECT_GAS_COMPENSATION;
        uint collToLiquidate = singleLiquidation.entireTroveColl.sub(singleLiquidation.collGasCompensation);

        (singleLiquidation.debtToOffset,
        singleLiquidation.collToSendToSP,
        singleLiquidation.debtToRedistribute,
        singleLiquidation.collToRedistribute) = _getOffsetAndRedistributionVals(singleLiquidation.entireTroveDebt, collToLiquidate, _NECTInStabPool);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        // If 100% < ICR < MCR, offset as much as possible, and redistribute the remainder
            ...
            singleLiquidation.collToSendToSP,
            singleLiquidation.debtToRedistribute,
            singleLiquidation.collToRedistribute) = _getOffsetAndRedistributionVals(singleLiquidation.entireTroveDebt, vars.collToLiquidate, _NECTInStabPool);
        ...
        * and there is NECT in the Stability Pool, only offset, with no redistribution,
        * but at a capped rate of 1.1 and only if the whole debt can be liquidated.
        * The remainder due to the capped rate will be claimable as collateral surplus.
        */
        } else if ((_ICR >= MCR) && (_ICR < _TCR) && (singleLiquidation.entireTroveDebt <= _NECTInStabPool)) {
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, vars.pendingDebtReward, vars.pendingCollReward);
            assert(_NECTInStabPool != 0);
    ...
    */
    ...
    {
        if (_NECTInStabPool > 0) {
        /*
        * Offset as much debt & collateral as possible against the Stability Pool, and redistribute the remainder
        * between all active troves.
        *
        *  If the trove's debt is larger than the deposited NECT in the Stability Pool:
        *
        *  - Offset an amount of the trove's debt equal to the NECT in the Stability Pool
        *  - Send a fraction of the trove's collateral to the Stability Pool, equal to the fraction of its offset debt
        *
        */
            debtToOffset = BeraBorrowMath._min(_debt, _NECTInStabPool);
    ...
    */
    function liquidateTroves(uint _n) external override {
        ContractsCache memory contractsCache = ContractsCache(
            activePool,
            defaultPool,
            INECTToken(address(0)),
            IPOLLENStaking(address(0)),
            sortedTroves,
        ...
        vars.NECTInStabPool = stabilityPoolCached.getTotalNECTDeposits();
        vars.recoveryModeAtStart = _checkRecoveryMode(vars.price);

        // Perform the appropriate liquidation sequence - tally the values, and obtain their totals
        if (vars.recoveryModeAtStart) {
            totals = _getTotalsFromLiquidateTrovesSequence_RecoveryMode(contractsCache, vars.price, vars.NECTInStabPool, _n);
        } else { // if !vars.recoveryModeAtStart
            totals = _getTotalsFromLiquidateTrovesSequence_NormalMode(contractsCache.activePool, contractsCache.defaultPool, vars.price, vars.NECTInStabPool, _n);
        }
        ...
        emit Liquidation(vars.liquidatedDebt, vars.liquidatedColl, totals.totalCollGasCompensation, totals.totalNECTGasCompensation);

        // Send gas compensation to caller
        _sendGasCompensation(contractsCache.activePool, msg.sender, totals.totalNECTGasCompensation, totals.totalCollGasCompensation);
    }
    ...
    */
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        vars.remainingNECTInStabPool = _NECTInStabPool;
        ...
        for (vars.i = 0; vars.i < _n && vars.user != firstUser; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                if (vars.ICR >= MCR && vars.remainingNECTInStabPool == 0) { break; }

                uint TCR = BeraBorrowMath._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _price);

                singleLiquidation = _liquidateRecoveryMode(_contractsCache.activePool, _contractsCache.defaultPool, vars.user, vars.ICR, vars.remainingNECTInStabPool, TCR, _price);

                // Update aggregate trackers
                vars.remainingNECTInStabPool = vars.remainingNECTInStabPool.sub(singleLiquidation.debtToOffset);
                ...
                singleLiquidation = _liquidateNormalMode(_contractsCache.activePool, _contractsCache.defaultPool, vars.user, vars.remainingNECTInStabPool);
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _n; vars.i++) {
            ...
            if (vars.ICR < MCR) {
                singleLiquidation = _liquidateNormalMode(_activePool, _defaultPool, vars.user, vars.remainingNECTInStabPool);
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidate_RecoveryMode(activePoolCached, defaultPoolCached, vars.price, vars.NECTInStabPool, _troveArray);
        } else {  //  if !vars.recoveryModeAtStart
            totals = _getTotalsFromBatchLiquidate_NormalMode(activePoolCached, defaultPoolCached, vars.price, vars.NECTInStabPool, _troveArray);
        }
        ...
        // Send gas compensation to caller
        _sendGasCompensation(activePoolCached, msg.sender, totals.totalNECTGasCompensation, totals.totalCollGasCompensation);
    }
    ...
    */
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _troveArray.length; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                if (vars.ICR >= MCR && vars.remainingNECTInStabPool == 0) { continue; }

                uint TCR = BeraBorrowMath._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _price);

                singleLiquidation = _liquidateRecoveryMode(_activePool, _defaultPool, vars.user, vars.ICR, vars.remainingNECTInStabPool, TCR, _price);
    ...
    internal pure returns(LiquidationTotals memory newTotals) {
        ...
        newTotals.totalNECTGasCompensation = oldTotals.totalNECTGasCompensation.add(singleLiquidation.NECTGasCompensation);
    ...
    // Redeem as much collateral as possible from _borrower's Trove in exchange for NECT up to _maxNECTamount
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        if (newDebt == NECT_GAS_COMPENSATION) {
            ...
            _redeemCloseTrove(_contractsCache, _borrower, NECT_GAS_COMPENSATION, newColl);
    ...
    */
    ...
    /* Send _NECTamount NECT to the system and redeem the corresponding amount of collateral from as many Troves as are needed to fill the redemption
    ...
    * redemption will stop after the last completely redeemed Trove and the sender will keep the remaining NECT amount, which they can attempt
    * to redeem later.
    */
        ...
        override
    {
        ...
        _requireAmountGreaterThanZero(_NECTamount);
        _requireNECTBalanceCoversRedemption(contractsCache.nectToken, msg.sender, _NECTamount);

        totals.totalNECTSupplyAtStart = getEntireSystemDebt();
        // Confirm redeemer's balance is less than total NECT supply
        assert(contractsCache.nectToken.balanceOf(msg.sender) <= totals.totalNECTSupplyAtStart);

        totals.remainingNECT = _NECTamount;
        ...
        // Loop through the Troves starting from the one with lowest collateral ratio until _amount of NECT is exchanged for collateral
        if (_maxIterations == 0) { _maxIterations = uint(-1); }
        while (currentBorrower != address(0) && totals.remainingNECT > 0 && _maxIterations > 0) {
            _maxIterations--;
    ...
    function getPendingNECTDebtReward(address _borrower) public view override returns (uint) {
        uint snapshotNECTDebt = rewardSnapshots[_borrower].NECTDebt;
        uint rewardPerUnitStaked = L_NECTDebt.sub(snapshotNECTDebt);

        if ( rewardPerUnitStaked == 0 || Troves[_borrower].status != Status.active) { return 0; }

        uint stake =  Troves[_borrower].stake;

        uint pendingNECTDebtReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);

        return pendingNECTDebtReward;
    }
    ...
    */
    ...
    function _addTroveOwnerToArray(address _borrower) internal returns (uint128 index) {
        /* Max array size is 2**128 - 1, i.e. ~3e30 troves. No risk of overflow, since troves have minimum NECT
        debt of liquidation reserve plus MIN_NET_DEBT. 3e30 NECT dwarfs the value of all wealth in the world ( which is < 1e15 USD). */
    ...
    */
    ...
    returns (bool)
    {
        uint TCR = BeraBorrowMath._computeCR(_entireSystemColl, _entireSystemDebt, _price);
    ...
    * 1) decays the baseRate based on time passed since last redemption or NECT borrowing operation.
    * then,
    ...
    */
    ...
    function getBorrowingFee(uint _NECTDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRate(), _NECTDebt);
    }

    function getBorrowingFeeWithDecay(uint _NECTDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRateWithDecay(), _NECTDebt);
    }

    function _calcBorrowingFee(uint _borrowingRate, uint _NECTDebt) internal pure returns (uint) {
        return _borrowingRate.mul(_NECTDebt).div(DECIMAL_PRECISION);
    }


    // Updates the baseRate state variable based on time elapsed since the last redemption or NECT borrowing operation.
    ...
    function _calcDecayedBaseRate() internal view returns (uint) {
        ...
        uint decayFactor = BeraBorrowMath._decPow(MINUTE_DECAY_FACTOR, minutesPassed);
    ...
    function _requireNECTBalanceCoversRedemption(INECTToken _nectToken, address _redeemer, uint _amount) internal view {
        require(_nectToken.balanceOf(_redeemer) >= _amount, "TroveManager: Requested redemption amount must be <= user's NECT token balance");
    }
