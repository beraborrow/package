...
/*
* PriceFeed for mainnet deployment, to be connected to Chainlink's live iBGT:USD aggregator reference 
...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    // Core BeraBorrow contracts
    address borrowerOperationsAddress;
    address troveManagerAddress;

    uint constant public iBGTUSD_TELLOR_REQ_ID = 1;
    ...
    */
    ...
    // The last good price seen from an oracle by BeraBorrow
    ...
    /*
    * fetchPrice():
    * Returns the latest price obtained from the Oracle. Called by BeraBorrow functions that require a current price.
    *
    * Also callable by anyone externally.
    *
    * Non-view function - it stores the last good price seen by BeraBorrow.
    *
    * Uses a main oracle (Chainlink) and a fallback oracle (Tellor) in case Chainlink fails. If both fail, 
    * it uses the last good price seen by BeraBorrow.
    *
    */
    ...
    */
    ...
    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        ...
        uint minPrice = BeraBorrowMath._min(currentScaledPrice, prevScaledPrice);
        uint maxPrice = BeraBorrowMath._max(currentScaledPrice, prevScaledPrice);
    ...
    function _bothOraclesSimilarPrice( ChainlinkResponse memory _chainlinkResponse, TellorResponse memory _tellorResponse) internal pure returns (bool) {
        ...
        uint minPrice = BeraBorrowMath._min(scaledTellorPrice, scaledChainlinkPrice);
        uint maxPrice = BeraBorrowMath._max(scaledTellorPrice, scaledChainlinkPrice);
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        /*
        * Convert the price returned by the Chainlink oracle to an 18-digit decimal for use by BeraBorrow.
        * At date of BeraBorrow launch, Chainlink uses an 8-digit price, but we also handle the possibility of
        * future changes.
        *
        */
        uint price;
        if (_answerDigits >= TARGET_DIGITS) {
            // Scale the returned price value down to BeraBorrow's target precision
            price = _price.div(10 ** (_answerDigits - TARGET_DIGITS));
        }
        else if (_answerDigits < TARGET_DIGITS) {
            // Scale the returned price value up to BeraBorrow's target precision
    ...
    function _getCurrentTellorResponse() internal view returns (TellorResponse memory tellorResponse) {
        try tellorCaller.getTellorCurrentValue(iBGTUSD_TELLOR_REQ_ID) returns
        (
            bool ifRetrieve,
            uint256 value,
