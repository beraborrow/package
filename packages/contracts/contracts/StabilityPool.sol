 ...
 * https://github.com/BeraborrowOfficial/beraborrow-frontend/blob/main/papers/Scalable_Reward_Distribution_with_Compounding_Stakes.pdf
 *
 ...
 *
 * We use the same mathematical product-sum approach to track POLLEN gains for depositors, where 'G' is the sum corresponding to POLLEN gains.
 * The product P (and snapshot P_t) is re-used, as the ratio P/P_t tracks a deposit's depletion due to liquidations.
 *
 */
contract StabilityPool is BeraBorrowBase, Ownable, CheckContract, IStabilityPool {
    ...
    * after a series of liquidations have occurred, each of which cancel some NECT debt with the deposit.
    *
    ...
    */
    ...
    /*
    * Similarly, the sum 'G' is used to calculate POLLEN gains. During it's lifetime, each deposit d_t earns a POLLEN gain of
    *  ( d_t * [G - G_t] )/P_t, where G_t is the depositor's snapshot of G taken at time t when  the deposit was made.
    *
    *  POLLEN reward events occur are triggered by depositor operations (new deposit, topup, withdrawal), and liquidations.
    *  In each case, the POLLEN reward is issued (i.e. G is updated), before other state changes are made.
    */
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        _sendNECTtoStabilityPool(msg.sender, _amount);

        uint newDeposit = compoundedNECTDeposit.add(_amount);
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        ...
        // Update front end stake
        uint compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint newFrontEndStake = compoundedFrontEndStake.sub(NECTtoWithdraw);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, msg.sender);

        _sendNECTToDepositor(msg.sender, NECTtoWithdraw);

        // Update deposit
        uint newDeposit = compoundedNECTDeposit.sub(NECTtoWithdraw);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    ...
    function _triggerPOLLENIssuance(ICommunityIssuance _communityIssuance) internal {
        uint POLLENIssuance = _communityIssuance.issuePOLLEN();
       _updateG(POLLENIssuance);
    }

    function _updateG(uint _POLLENIssuance) internal {
        uint totalNECT = totalNECTDeposits; // cached to save an SLOAD
        /*
        * When total deposits is 0, G is not updated. In this case, the POLLEN issued can not be obtained by later
        * depositors - it is missed out on, and remains in the balanceof the CommunityIssuance contract.
        *
        */
        if (totalNECT == 0 || _POLLENIssuance == 0) {return;}

        uint POLLENPerUnitStaked;
        POLLENPerUnitStaked =_computePOLLENPerUnitStaked(_POLLENIssuance, totalNECT);

        uint marginalPOLLENGain = POLLENPerUnitStaked.mul(P);
        epochToScaleToG[currentEpoch][currentScale] = epochToScaleToG[currentEpoch][currentScale].add(marginalPOLLENGain);

        emit G_Updated(epochToScaleToG[currentEpoch][currentScale], currentEpoch, currentScale);
    }

    function _computePOLLENPerUnitStaked(uint _POLLENIssuance, uint _totalNECTDeposits) internal returns (uint) {
        /*  
        * Calculate the POLLEN-per-unit staked.  Division uses a "feedback" error correction, to keep the 
        ...
        */
        uint POLLENNumerator = _POLLENIssuance.mul(DECIMAL_PRECISION).add(lastPOLLENError);

        uint POLLENPerUnitStaked = POLLENNumerator.div(_totalNECTDeposits);
        lastPOLLENError = POLLENNumerator.sub(POLLENPerUnitStaked.mul(_totalNECTDeposits));

        return POLLENPerUnitStaked;
    }
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        uint totalNECT = totalNECTDeposits; // cached to save an SLOAD
        if (totalNECT == 0 || _debtToOffset == 0) { return; }
        ...
        returns (uint iBGTGainPerUnitStaked, uint NECTLossPerUnitStaked)
    {
        ...
        */
        ...
        if (_debtToOffset == _totalNECTDeposits) {
            NECTLossPerUnitStaked = DECIMAL_PRECISION;  // When the Pool depletes to 0, so does each deposit 
            lastNECTLossError_Offset = 0;
        } else {
            uint NECTLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(lastNECTLossError_Offset);
            /*
            * Add 1 to make error in quotient positive. We want "slightly too much" NECT loss,
            * which ensures the error in any given compoundedNECTDeposit favors the Stability Pool.
            */
            NECTLossPerUnitStaked = (NECTLossNumerator.div(_totalNECTDeposits)).add(1);
    ...
    function _updateRewardSumAndProduct(uint _iBGTGainPerUnitStaked, uint _NECTLossPerUnitStaked) internal {
        ...
        assert(_NECTLossPerUnitStaked <= DECIMAL_PRECISION);
        /*
        * The newProductFactor is the factor by which to change all deposits, due to the depletion of Stability Pool NECT in the liquidation.
        * We make the product factor 0 if there was a pool-emptying. Otherwise, it is (1 - NECTLossPerUnitStaked)
        */
        uint newProductFactor = uint(DECIMAL_PRECISION).sub(_NECTLossPerUnitStaked);
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        // Cancel the liquidated NECT debt with the NECT in the stability pool
        activePoolCached.decreaseNECTDebt(_debtToOffset);
        _decreaseNECT(_debtToOffset);
    ...
    function _decreaseNECT(uint _amount) internal {
        uint newTotalNECTDeposits = totalNECTDeposits.sub(_amount);
        totalNECTDeposits = newTotalNECTDeposits;
    ...
    * Given by the formula:  POLLEN = d0 * (G - G(0))/P(0)
    * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
    * d0 is the last recorded deposit value.
    */
    function getDepositorPOLLENGain(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {return 0;}
        ...
        */
        ...
        uint POLLENGain = kickbackRate.mul(_getPOLLENGainFromSnapshots(initialDeposit, snapshots)).div(DECIMAL_PRECISION);

        return POLLENGain;
    }

    /*
    * Return the POLLEN gain earned by the front end. Given by the formula:  E = D0 * (G - G(0))/P(0)
    * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
    *
    * D0 is the last recorded value of the front end's total tagged deposits.
    */
    function getFrontEndPOLLENGain(address _frontEnd) public view override returns (uint) {
        ...
        uint POLLENGain = frontEndShare.mul(_getPOLLENGainFromSnapshots(frontEndStake, snapshots)).div(DECIMAL_PRECISION);
        return POLLENGain;
    }

    function _getPOLLENGainFromSnapshots(uint initialStake, Snapshots memory snapshots) internal view returns (uint) {
       /*
        * Grab the sum 'G' from the epoch at which the stake was made. The POLLEN gain may span up to one scale change.
        * If it does, the second portion of the POLLEN gain is scaled by 1e9.
        * If the gain spans no scale change, the second portion will be 0.
        */
        uint128 epochSnapshot = snapshots.epoch;
        uint128 scaleSnapshot = snapshots.scale;
        ...
        uint POLLENGain = initialStake.mul(firstPortion.add(secondPortion)).div(P_Snapshot).div(DECIMAL_PRECISION);
    ...
    */
    function getCompoundedNECTDeposit(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) { return 0; }
    ...
    */
    ...
    function _sendNECTtoStabilityPool(address _address, uint _amount) internal {
        nectToken.sendToPool(_address, address(this), _amount);
        uint newTotalNECTDeposits = totalNECTDeposits.add(_amount);
        totalNECTDeposits = newTotalNECTDeposits;
        emit StabilityPoolNECTBalanceUpdated(newTotalNECTDeposits);
    }
    ...
    function _sendNECTToDepositor(address _depositor, uint NECTWithdrawal) internal {
        if (NECTWithdrawal == 0) {return;}

        nectToken.returnFromPool(address(this), _depositor, NECTWithdrawal);
        _decreaseNECT(NECTWithdrawal);
    }
    ...
    function _payOutPOLLENGains(ICommunityIssuance _communityIssuance, address _depositor, address _frontEnd) internal {
        // Pay out front end's POLLEN gain
        if (_frontEnd != address(0)) {
            uint frontEndPOLLENGain = getFrontEndPOLLENGain(_frontEnd);
            _communityIssuance.sendPOLLEN(_frontEnd, frontEndPOLLENGain);
            emit POLLENPaidToFrontEnd(_frontEnd, frontEndPOLLENGain);
        }

        // Pay out depositor's POLLEN gain
        uint depositorPOLLENGain = getDepositorPOLLENGain(_depositor);
        _communityIssuance.sendPOLLEN(_depositor, depositorPOLLENGain);
        emit POLLENPaidToDepositor(_depositor, depositorPOLLENGain);
    }
