...
async function main() {
  ...
  console.log(`whale address from import: ${iBGT_WHALE}`)

  // Get the iBGT whale signer 
  const whale = await ethers.provider.getSigner(iBGT_WHALE)
  console.log(`whale addr : ${await whale.getAddress()}`)
  console.log(`whale iBGT balance: ${ await ethers.provider.getBalance(whale.getAddress())}`)

  // Send iBGT to the deployer's address
