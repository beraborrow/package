...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    nectToken = await NECTToken.at(await echidnaTester.nectToken())
  ...
  it.only('NECT balance', async () => {
    await echidnaTester.openTroveExt('0', '0', '4210965169908805439447313562489173090')

    const totalSupply = await nectToken.totalSupply();
    const gasPoolBalance = await nectToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getNECTDebt();
    const defaultPoolBalance = await defaultPool.getNECTDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalNECTDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = nectToken.balanceOf(currentTrove);
