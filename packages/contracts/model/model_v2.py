...
class ModelParams:
    def __init__(self):
        ...
        self.lookback = 5 # Lookback parameter for iBGT price momentum
...
### Functions
def get_new_momentum(data, params, iBGT_price):
    lookback = params.lookback
    if lookback == 0:
        return 0

    iBGT_price_past = get_past_iBGT_price(data, params)

    new_momentum = (iBGT_price - iBGT_price_past) /  iBGT_price_past
    return new_momentum

def get_past_iBGT_price(data, params):
    length = len(data.iBGT_price)
    
    iBGT_price_past = None
    if (params.lookback > length):
        iBGT_price_past = data.iBGT_price[0]
    else:
        iBGT_price_past = data.iBGT_price[length - params.lookback - 1]

    if iBGT_price_past == 0:
        return 1
    return iBGT_price_past
...
# return the exogenous market demand for holding POLLEN tokens. Assume constant. Could be a function of:
...
# Given BeraBorrow's hard price ceiling of 1.10, 
...
### Various iBGT price functions

def constant_iBGT_price(last_price):
    return last_price

# iBGT price generator is a random walk (normal dist.), with occasional large +ve and -ve jumps
def randomwalk_iBGT_price(last_price):
    big_event = 0
...
def linear_increasing_iBGT_price(last_price, gradient):
    return last_price + gradient

def oscillating_iBGT_price(min, magnitude, i):
    return min + magnitude + magnitude*np.sin(i)

def linear_decreasing_iBGT_price(start, gradient, i):
    val = (start - (gradient*i))
    if val <= 0:
        return 0
    return val

def one_over_i_iBGT_price(scale, i):
    return scale/i

def quadratic_iBGT_price(min, scale, i):
    return min + scale*(i**2)

def sublinear_iBGT_price(last_price, steepness, i):
...
for i in range(1, 250):
    last_iBGT_price =  data.iBGT_price[-1]

    # update exogenous iBGT price

    # iBGT_price = last_iBGT_price
    iBGT_price = randomwalk_iBGT_price(last_iBGT_price)
    # iBGT_price = oscillating_iBGT_price(500, 100, i)
    # iBGT_price = quadratic_iBGT_price(500, 10, i)
    # iBGT_price = linear_increasing_iBGT_price(last_iBGT_price, 3)
    # iBGT_price = linear_decreasing_iBGT_price(800, 1, i)
    # iBGT_price = one_over_i_iBGT_price(1000, i)
    # iBGT_price = sublinear_iBGT_price(last_iBGT_price, 10, i)
    
    momentum = get_new_momentum(data, params, iBGT_price)
    ...
    data.iBGT_price.append(iBGT_price)
...
ax3.set_title('iBGT Price')
plt.ylim(0, 1000)
plt.plot(data.iBGT_price)
