...
export interface PollenPriceResponse {
  pollenPriceUSD: Decimal;
}

export const fetchPollenPrice = async (): Promise<PollenPriceResponse> => {
  const response = await fetchCoinGeckoSimplePrice(["beraborrow"] as const, ["usd"] as const);

  return {
    pollenPriceUSD: Decimal.from(response.beraborrow.usd)
  };
};
