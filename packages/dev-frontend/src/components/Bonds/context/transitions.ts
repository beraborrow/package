...
export enum BNectAmmTokenIndex {
  BNECT,
  NECT,
  BNECT_NECT_LP
}
...
export type Addresses = {
  BNECT_AMM_ADDRESS: Address;
  BNECT_AMM_STAKING_ADDRESS: Address;
  BNECT_TOKEN_ADDRESS: Address;
  BOND_NFT_ADDRESS: Address;
  CHICKEN_BOND_MANAGER_ADDRESS: Address;
  NECT_OVERRIDE_ADDRESS: Address;
  BNECT_LP_ZAP_ADDRESS: Address;
};

// This payload is only dispatched by "Manage liquidity"
export type ApprovePressedPayload = {
  tokensNeedingApproval: Map<BNectAmmTokenIndex, Address>;
};
...
export type BNectLpRewards = Array<{ name: string; address: string; amount: Decimal }>;
